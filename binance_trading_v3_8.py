...
- Corrección en extract_symbol_filters (usa NOTIONAL)
- Mejora en clasificación de pullback_quality (persistencia 15m)
...
def is_support_reliable(price_level: float, candles: List[Dict[str, Any]], volume_threshold: float = 10000) -> bool:
    ...
    if not candles or price_level <= EPS:
        return True  # Si no hay datos, asumimos que es confiable (evita falsos negativos)
    
    # Revisar las últimas 20 velas o todas si hay menos
    ...
    for candle in check_candles:
        ...
        # Si el low está dentro del 1% del nivel de soporte
        if abs(low - price_level) / price_level < 0.01:
            if volume > volume_threshold:
                touches += 1
    
    # Requerimos al menos 2 toques con volumen significativo
    return touches >= 2
...
def timeframe_summary(
    ...
    # Filtrar soportes por volumen confiable
    zone_candidates = [
        ("ma25", ma25),
        ("ma99", ma99),
...
def build_position_operational_summary(
    ...
    # Advertencia si el precio está cerca del stop loss
...
def suggest_limit_buy(
    ...
    note_parts = [
        ...
        "La selección de entradas 3.8 prioriza zonas operativas, fuerza escalones distintos (aggressive/base/conservative), exige separación mínima entre escalones y devuelve null cuando no existen niveles realmente distintos."
    ]
...
# =========================
# Enriquecimiento de watchlist v3.8
# =========================
...
def classify_pullback_quality(
    ...
    # Mejora: verificar persistencia en 15m (al menos 2 de las últimas 3 velas)
    weak_15m = False
...
def build_watchlist_text(payload: Dict[str, Any]) -> str:
    ...
    lines.append("RANKING (screening + operabilidad):")
    for idx, item in enumerate(payload["ranking"], start=1):
        lines.append(
            f"{idx}. {item['symbol']} | score={item['score']} ({item.get('score_bucket')}) | trend={item.get('trend_score')} | tradeability={item.get('tradeability_score')} | estado={((item.get('setup_status') or {}).get('state'))} | precio={item['last_price']} | compra_limite={item['suggested_limit_buy']} | ref={item['limit_reference']}"
        )
        lines.append(
    ...
    lines.append("NOTA:")
    lines.append("  El ranking es un screening operativo. Úsalo para priorizar revisión humana, no como orden automática. rr_operativa_preliminar y rr_estructural_preliminar son referencias iniciales, no un R:R definitivo.")
    return "\n".join(lines)
...
def save_ranking_history(ranking: List[Dict[str, Any]], symbols: List[str], capital: float, output_dir: Path) -> None:
    ...
    # Actualizar archivo consolidado (últimos 30 rankings)
...
def run_position_mode(args: argparse.Namespace) -> int:
    ...
    # Barra de progreso para modo posición (consistencia con mercado)
...
def run_market_mode(args: argparse.Namespace) -> int:
    ...
    print("Generando archivos finales de watchlist...", flush=True)
    ...
    # Guardar historial de ranking
    save_ranking_history(ranking_sorted, symbols, args.capital, output_dir)

    print(render_progress_bar(total_symbols, total_symbols) + " | Watchlist finalizada", flush=True)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Binance Trading v3.8: modo posición (OCO) y modo mercado (watchlist avanzado)."
    )
