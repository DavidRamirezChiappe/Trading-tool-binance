...
def resolve_binance_credentials(env_file_arg: Optional[str]) -> Tuple[str, str, str]:
    ...
    2. Archivo .env indicado por --archivo-env
    3. Archivo .env en la misma carpeta del script
    4. Archivo .env en el directorio actual
    """
    ...
    for b in account.get("balances", []):
        if b["asset"] in {base_asset, quote_asset}:
            ...
            balances_map[b["asset"]] = {
                "free": b["free"],
                "locked": b["locked"],
                "total": normalize_number_str(free_val + locked_val),
            }
...
# =========================
# Sugerencias de compra / ranking watchlist
# =========================

def suggest_limit_buy(public_data: Dict[str, Any]) -> Dict[str, Any]:
    ticker = public_data["ticker"]
    tfs = public_data["timeframes"]
    ...
    def add_candidate(name: str, value: Optional[float]) -> None:
        if value is not None and value < last_price:
            candidates.append((name, value))

    add_candidate("15m_recent_low", safe_float(tfs["15m"].get("recent_low")))
    add_candidate("1h_ma25", safe_float(tfs["1h"].get("ma25")))
    add_candidate("1h_recent_low", safe_float(tfs["1h"].get("recent_low")))
    add_candidate("1h_ma99", safe_float(tfs["1h"].get("ma99")))
    add_candidate("4h_ma25", safe_float(tfs["4h"].get("ma25")))
    add_candidate("4h_ma99", safe_float(tfs["4h"].get("ma99")))
    add_candidate("4h_recent_low", safe_float(tfs["4h"].get("recent_low")))
    ...
    primary_name, primary_value = candidates_sorted[0]
    second_name, second_value = candidates_sorted[1] if len(candidates_sorted) > 1 else (None, None)

    distance_pct = ((last_price / primary_value) - 1.0) * 100.0 if primary_value > EPS else None

    return {
        "suggested_limit_buy": floor_to_step(primary_value, tick_size),
        "reference": primary_name,
        "second_support": {
            "name": second_name,
            "value": normalize_number_str(second_value),
        } if second_name else None,
        "distance_pct_from_last": normalize_number_str(distance_pct),
        "note": "Sugerencia mecánica basada en soportes cercanos por debajo del precio actual. No es una señal automática.",
    }


def score_rebound_candidate(symbol: str, public_data: Dict[str, Any]) -> Dict[str, Any]:
    ticker = public_data["ticker"]
    ...
    if ma25_4 is not None and ma99_4 is not None:
        if ma25_4 > ma99_4:
            ...
            score -= 2
            reasons.append("sesgo débil en 4h (MA25 <= MA99)")
    ...
    if c1 is not None and ma25_1 is not None:
        ...
        if dist_1h_ma25 is not None and -2.5 <= dist_1h_ma25 <= 1.0:
            score += 2
            reasons.append("retroceso cercano a MA25 de 1h")
    ...
    if spread_pct is not None:
        if spread_pct <= 0.08:
            score += 1
            reasons.append("spread corto")
        elif spread_pct > 0.30:
            score -= 2
            reasons.append("spread amplio")

    bid_notional = safe_float(depth.get("bid_notional_top10")) or 0.0
    ask_notional = safe_float(depth.get("ask_notional_top10")) or 0.0
    if bid_notional > 50000 and ask_notional > 50000:
        score += 1
        reasons.append("liquidez decente top10")
    ...
    return {
        ...
        "score": score,
        "reasons": reasons,
        "suggested_limit_buy": suggestion.get("suggested_limit_buy"),
...
def build_position_analysis_text(summary: Dict[str, Any]) -> str:
    ...
    for tf in ["15m", "1h", "4h"]:
        ...
        lines.append(f"  Dist vs MA7 (%): {tf_data['dist_pct_vs_ma7']}")
        lines.append(f"  Dist vs MA25 (%): {tf_data['dist_pct_vs_ma25']}")
        lines.append(f"  Dist vs MA99 (%): {tf_data['dist_pct_vs_ma99']}")
        lines.append("")
...
def build_watchlist_text(payload: Dict[str, Any]) -> str:
    ...
    for idx, item in enumerate(payload["ranking"], start=1):
        lines.append(f"{idx}. {item['symbol']} | score={item['score']} | precio={item['last_price']} | compra_limite={item['suggested_limit_buy']} | ref={item['limit_reference']}")
        for reason in item["reasons"]:
            lines.append(f"   - {reason}")
        lines.append(f"   - distancia_vs_precio_actual: {item['distance_pct_from_last']}%")
...
def run_position_mode(args: argparse.Namespace) -> int:
    ...
    print("- klines_4h.csv")
    if args.include_private:
        print(f"- private_data incluido: balances, trades, openOrders y openOrderList")
...
def run_market_mode(args: argparse.Namespace) -> int:
    ...
    for symbol in symbols:
        ...
        ranking.append(score_rebound_candidate(symbol, public_data))
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Binance Trading v3: modo posición (OCO) y modo mercado (watchlist)."
    )
