...
def compute_oco_viability(
    ...
    if stop_air_atr is not None:
        if stop_air_atr < 0.65:
            score -= 22
            stop_air_quality = "sin_aire"
            reasons.append("stop con poco aire (<0.65 ATR)")
        elif stop_air_atr <= 2.4:
            score += 14
            stop_air_quality = "suficiente"
            reasons.append("stop con aire técnico suficiente")
        elif stop_air_atr <= 4.0:
            score -= 2
...
def build_watchlist_text(payload: Dict[str, Any]) -> str:
    ...
    if hist:
        lines.append("CONTEXTO HISTÓRICO v4.2.1:")
...
def passes_hard_filters(candidate: Dict[str, Any]) -> Tuple[bool, List[str]]:
    ...
    stop_air_quality = (((candidate.get("oco_viability") or {}).get("stop_air_quality")) or "").lower()
    ...
    if reward_pct is None or reward_pct < 1.25:
    ...
    if not tp_tactico or not stop_tactico:
        ...
        rr_min = 0.95 if (setup_state == "degradado" and is_fallback) else 1.15
        if rr_tactico < rr_min:
            reasons.append("rr_tactico_insuficiente")

    if fill_score is not None and fill_score < 35:
        reasons.append("fill_probability_baja")
    if oco_score is not None and oco_score < 40:
        reasons.append("oco_viability_baja")
    if ev_score is not None and ev_score < 45:
        reasons.append("expected_value_bajo")
    if stop_air_quality in {"sin_aire", "demasiado_profundo"}:
...
def save_ranking_history(
    ...
    v4.2.1 conserva compatibilidad con el histórico anterior, pero desde esta versión
...
def run_market_mode(args: argparse.Namespace) -> int:
    ...
    print("Generando archivos finales de oportunidades v4.2...", flush=True)
    ...
    print(render_progress_bar(total_symbols, total_symbols) + " | Análisis v4.2.1 finalizado", flush=True)
    print(f"OK. Análisis v4.2.1 generado en: {output_dir.resolve()}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Binance Trading v4.2.1: modo posición (OCO), modo mercado y auditoría histórica enriquecida."
    )
    ...
    p_mkt = subparsers.add_parser(
        "mercado",
        help="Analiza varios pares y devuelve una lista única de oportunidades v4.2.1."
    )
