...
def suggest_limit_buy(
    ...
    note_parts = [
        ...
        "La selección de entradas 3.9 prioriza zonas operativas, fuerza escalones distintos (aggressive/base/conservative), exige separación mínima entre escalones y devuelve null cuando no existen niveles realmente distintos."
    ]
...
def build_watchlist_text(payload: Dict[str, Any]) -> str:
    ...
    for idx, item in enumerate(payload["ranking"], start=1):
        lines.append(
            f"{idx}. {item['symbol']} | score={item['score']} ({item.get('score_bucket')}) | raw={item.get('score_raw')} | trend={item.get('trend_score')} | tradeability={item.get('tradeability_score')} | estado={((item.get('setup_status') or {}).get('state'))} | precio={item['last_price']} | compra_limite={item['suggested_limit_buy']} | ref={item['limit_reference']}"
        )
        lines.append(
...
def run_market_mode(args: argparse.Namespace) -> int:
    ...
    # Filtrar solo vigentes si se solicitó
    if args.only_vigent:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Binance Trading v3.9: modo posición (OCO) y modo mercado (watchlist avanzado)."
    )
