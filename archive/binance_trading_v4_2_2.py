...
def build_watchlist_text(payload: Dict[str, Any]) -> str:
    ...
    if hist:
        lines.append("CONTEXTO HISTÓRICO v4.2.2:")
    ...
    if regime:
        lines.append("RÉGIMEN DE MERCADO v4.2.2:")
...
def ensure_snapshots_layout(root_dir: Path) -> Dict[str, Path]:
    ...
    No crea carpetas de ejecución directamente bajo Snapshots/.
    """
    ...
    for key in ("historial", "mercado", "posicion"):
...
# =========================
# v4.2.2 - régimen, modos de trade y journal simple
# =========================
...
def save_ranking_history(
    ...
    """Guarda histórico enriquecido del ranking en Snapshots/Historial.

    v4.2.2 conserva compatibilidad con el histórico anterior, pero desde esta versión
    el consolidado guarda features suficientes para auditar conservadurismo, fill y OCO.
    """
    layout = ensure_snapshots_layout(snapshots_root)
    ...
    write_jsonl(history_dir / f"ranking_features_{timestamp}.jsonl", all_compact)
...
def run_market_mode(args: argparse.Namespace) -> int:
    ...
    print("Generando archivos finales de oportunidades v4.2.2...", flush=True)
    ...
    print(render_progress_bar(total_symbols, total_symbols) + " | Análisis v4.2.2 finalizado", flush=True)
    print(f"OK. Análisis v4.2.2 generado en: {output_dir.resolve()}")
    ...
    print(f"- Historial guardado en: {(snapshots_root / 'Historial').resolve()}")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Binance Trading v4.2.2: trade_mode, risk_off, cooldown, journal simple y filtros swing defensivos."
    )
    ...
    p_mkt = subparsers.add_parser(
        "mercado",
        help="Analiza varios pares y devuelve una lista única de oportunidades v4.2.2."
    )
