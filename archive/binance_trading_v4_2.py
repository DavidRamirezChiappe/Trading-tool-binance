...
def save_ranking_history(ranking: List[Dict[str, Any]], symbols: List[str], capital: float, snapshots_root: Path) -> None:
    """Guarda un registro histórico del ranking en Snapshots/Historial."""
...
def run_market_mode(args: argparse.Namespace) -> int:
    ...
    save_ranking_history(visible_ranking, symbols, args.capital, snapshots_root)

    print(render_progress_bar(total_symbols, total_symbols) + " | Análisis v4.2 finalizado", flush=True)
    print(f"OK. Análisis v4.2 generado en: {output_dir.resolve()}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Binance Trading v4.2.0: modo posición (OCO) y modo mercado (watchlist ejecutable + viabilidad OCO + probabilidad de fill)."
    )
