...
def timeframe_summary(
    ...
    zone_values = [float(v) for _, v in zone_candidates if v is not None and last_close is not None and v <= last_close]
    support_zone = None
    if zone_values:
        support_zone = {
            "low": min(zone_values),
...
def build_invalidation_levels(public_data: Dict[str, Any], entry_price: Optional[float] = None) -> Dict[str, Any]:
    ...
    base_stop_name, base_stop_value = min(valid, key=lambda x: x[1])
    trigger_reference = max(v for _, v in valid if v <= base_stop_value * 1.01)

    stop_candidate = base_stop_value
    trigger_candidate = max(base_stop_value, trigger_reference)

    risk_pct = None
    if entry_price is not None and stop_candidate is not None and entry_price > EPS:
        risk_pct = ((entry_price / stop_candidate) - 1.0) * 100.0

    return {
        "stop_candidate": format_price(stop_candidate, tick_size),
        "trigger_candidate": format_price(trigger_candidate, tick_size),
        "reason": f"Invalidación mecánica basada en pérdida del soporte más profundo relevante ({base_stop_name}).",
        "risk_pct_from_entry": format_pct(risk_pct),
        "reference_levels": [
...
def suggest_limit_buy(
    ...
    aggressive = pick_best_by_names(
        [c for c in candidates_sorted if c["distance_pct"] >= max(0.35, min_distance_pct * 0.7)],
        aggressive_pref,
    ) or candidates_sorted[0]

    used_names = {aggressive["name"]}

    base_entry = pick_best_by_names(candidates_sorted, base_pref, used_names)
    if base_entry is None:
        base_entry = pick_distinct_candidate(candidates_sorted, used_names, min_distance_pct=min_distance_pct)
    if base_entry is None:
        base_entry = chosen
    used_names.add(base_entry["name"])

    conservative = pick_best_by_names(candidates_sorted, conservative_pref, used_names)
    if conservative is None:
        conservative = pick_distinct_candidate(candidates_sorted, used_names, prefer_farthest=True)
    if conservative is None:
        conservative = max(candidates_sorted, key=lambda x: x["distance_pct"])

    if base_entry["distance_pct"] < min_distance_pct:
        alt = pick_distinct_candidate(
            [c for c in candidates_sorted if c["distance_pct"] >= min_distance_pct],
            {aggressive["name"]},
    ...
    if candidate_price_key(float(aggressive["value"]), tick_size) == candidate_price_key(float(base_entry["value"]), tick_size):
        alt = pick_distinct_candidate(candidates_sorted, {aggressive["name"]}, min_distance_pct=max(0.45, min_distance_pct * 0.7))
        if alt is not None:
            base_entry = alt

    if candidate_price_key(float(conservative["value"]), tick_size) in {
        candidate_price_key(float(aggressive["value"]), tick_size),
        candidate_price_key(float(base_entry["value"]), tick_size),
    }:
        alt = pick_distinct_candidate(candidates_sorted, {aggressive["name"], base_entry["name"]}, prefer_farthest=True)
        if alt is not None:
            conservative = alt

    if safe_float(base_entry.get("value")) is not None and safe_float(conservative.get("value")) is not None:
        if float(base_entry["value"]) < float(conservative["value"]):
            base_entry, conservative = conservative, base_entry
    ...
    note_parts = [
        ...
        "La selección de entradas 3.4 fuerza escalones distintos (aggressive/base/conservative) cuando el mercado ofrece niveles utilizables."
    ]
    if base_entry["distance_pct"] < min_distance_pct:
        note_parts.append("La entrada base está bastante pegada al precio actual.")
    elif base_entry["distance_pct"] > max_distance_pct:
    ...
    result = {
        "suggested_limit_buy": floor_to_step(base_entry["value"], tick_size),
        "reference": base_entry["name"],
        "second_support": {
            "name": second["name"],
            "value": normalize_number_str(second["value"], 8),
            "distance_pct": normalize_number_str(second["distance_pct"], 4),
        } if second else None,
        "distance_pct_from_last": normalize_number_str(base_entry["distance_pct"], 4),
        "entries": {
            "aggressive": {
...
def score_rebound_candidate(symbol: str, public_data: Dict[str, Any], capital_quote: float = 35.0) -> Dict[str, Any]:
    ...
    return {
        ...
        "entries": suggestion.get("entries"),
...
def build_position_analysis_text(summary: Dict[str, Any]) -> str:
    ...
    if invalidation:
        ...
        lines.append(f"  trigger_candidate: {invalidation.get('trigger_candidate')}")
        lines.append(f"  stop_candidate: {invalidation.get('stop_candidate')}")
        lines.append(f"  risk_pct_from_entry: {invalidation.get('risk_pct_from_entry')}")
...
def build_watchlist_text(payload: Dict[str, Any]) -> str:
    ...
    for idx, item in enumerate(payload["ranking"], start=1):
        lines.append(
            f"{idx}. {item['symbol']} | score={item['score']} | estado={((item.get('setup_status') or {}).get('state'))} | precio={item['last_price']} | "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Binance Trading v3.3: modo posición (OCO) y modo mercado (watchlist)."
    )
